...
class AdvertPage extends Component {
  ...
  getPoster = (posterEmail) => {
  	axios.get(`http://localhost:4000/poster/${posterEmail}`)
  ...
  renderAdvert= (postedByUser, advert, tradefor) => {
  	...
  	return (
  		<div>
  			<img src={advert.imgurl || 'https://via.placeholder.com/200x200'} alt='item' className='advertimg'/>
  			<div className='details'>
  				<h2>{advert.title}</h2>
  				<p>Posted by: <Link to={`/users/${poster.id}`}>{poster.username}</Link></p>
  				<p>Location: {advert.postcode}</p>
  				<p>{advert.description}</p>
  				<p>Category: {advert.category}</p>
  				<p>Item condition: {advert.condition}</p>
